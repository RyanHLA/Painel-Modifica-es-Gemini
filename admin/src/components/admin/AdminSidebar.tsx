...
const AdminSidebar = ({ activeTab, onTabChange, onSignOut }: AdminSidebarProps) => {
  ...
  return (
    <aside className="fixed left-0 top-0 z-40 h-screen w-[280px]">
      <div className="w-[280px] bg-zinc-950 border-r border-zinc-800/50 h-full p-4 flex flex-col">

        {/* Logo */}
        <div className="flex items-center mb-8 px-4 mt-2">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="583 1302 2344 787" className="h-6 w-auto">
            {/* F */}
            ...
            return (
              <div key={index} className="flex flex-col">
                ...
                <div
                  ...
                  className={cn(
                    'group flex items-center px-4 py-[10px] rounded-[14px] cursor-pointer transition-all duration-200 ease-in-out border-l-2',
                    isExactMatch
                ...
                >
                  ...
                  <span className={cn(
                    'flex-1 text-[15px] tracking-tight transition-all duration-200',
        ...
        <div ref={profileMenuRef} className="mt-auto pt-4 border-t border-zinc-800/50 relative">
          ...
          {/* Dropdown */}
          {isProfileMenuOpen && (
            <div className="absolute bottom-[calc(100%+8px)] left-0 w-full bg-zinc-900 border border-zinc-800 rounded-[16px] shadow-2xl p-1.5 z-50 animate-in fade-in slide-in-from-bottom-2">
          ...
          <div
            ...
            className={cn(
              'flex items-center justify-between px-2 py-2.5 rounded-[14px] cursor-pointer transition-colors duration-200 group',
              isProfileMenuOpen ? 'bg-zinc-800/50' : 'hover:bg-zinc-800/30'
            )}
          >
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 bg-white rounded-[10px] flex items-center justify-center text-black font-bold text-sm shadow-[0_0_10px_rgba(255,255,255,0.1)] group-hover:scale-105 transition-transform duration-200">
                E
              </div>
              <div className="flex flex-col">
                <span className="text-zinc-100 font-semibold text-[14px] tracking-tight leading-tight">
                  Evelyn Cooper
                </span>
                <span className="text-zinc-500 font-medium text-[12px] tracking-tight mt-0.5">
                  Fotux Pro
                </span>
              </div>
            </div>
            ...
            <div className={cn(
              'text-zinc-500 group-hover:text-zinc-300 transition-all duration-300',
